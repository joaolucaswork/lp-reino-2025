...
(() => {
  ...
  var CARD_INFO_SELECTORS = {
    ["bg-text-color" /* BG_TEXT_COLOR */]: '[card-info="bg-text-color"]',
    ["name" /* NAME */]: '[card-info="name"]',
    ["title" /* TITLE */]: '[card-info="title"]',
    ["elements" /* ELEMENTS */]: '[card-info="elements"]',
    ["mini" /* MINI */]: '[card-info="mini"]',
    ["date" /* DATE */]: '[card-info="date"]',
    ["ilustration" /* ILUSTRATION */]: '[card-info="ilustration"]'
  };
  var CardInfoMapper = class {
    elements;
    constructor() {
    ...
    queryAllElements() {
      return {
        wrapper: document.querySelector(CARD_INFO_SELECTORS["bg-text-color" /* BG_TEXT_COLOR */]),
        name: document.querySelector(CARD_INFO_SELECTORS["name" /* NAME */]),
        title: document.querySelector(CARD_INFO_SELECTORS["title" /* TITLE */]),
        elements: document.querySelector(CARD_INFO_SELECTORS["elements" /* ELEMENTS */]),
        mini: document.querySelector(CARD_INFO_SELECTORS["mini" /* MINI */]),
        date: document.querySelector(CARD_INFO_SELECTORS["date" /* DATE */]),
        ilustration: document.querySelector(CARD_INFO_SELECTORS["ilustration" /* ILUSTRATION */])
      };
    }
    /**
    ...
    getElement(attribute) {
      const elementMap = {
        ["bg-text-color" /* BG_TEXT_COLOR */]: "wrapper",
        ["name" /* NAME */]: "name",
        ["title" /* TITLE */]: "title",
  ...
  var investorTitles = [
    ...
    "Investidor Estrat\xE9gico",
    "Investidor Vision\xE1rio",
    "Investidor Inovador",
    "Investidor Consciente",
    "Investidor Anal\xEDtico",
    "Investidor Empreendedor",
    "Investidor Resiliente"
  ];
  var startTitleRotation = (mapper, intervalMs = 3e3) => {
    let currentIndex = 0;
    const rotateTitle = () => {
  ...
  var initCardUpdates = (mapper, titleIntervalMs = 3e3) => {
  ...
  var CardRotationManager = class {
    ...
    getRotationState() {
      ...
      return hasRotateClass ? "back" /* BACK */ : "front" /* FRONT */;
    }
    /**
     ...
     */
    isRotated() {
      return this.getRotationState() === "back" /* BACK */;
    }
    /**
  ...
  var LogoCardToggle = class {
    ...
    attachEventListeners() {
      const logoElements = document.querySelectorAll(this.config.logoSelector);
     ...
     */
    dispatchRotationEvent(newState) {
      const event = new CustomEvent("card-rotation-toggle", {
        detail: {
          state: newState,
          isRotated: newState === "back" /* BACK */
        },
        bubbles: true
      });
  ...
  function initLogoCardToggle(config = {}) {
  ...
  function initProfileCardTilt(config) {
    ...
    const cardElement = document.querySelector(settings.cardSelector);
    if (!cardElement) {
      if (settings.debug) {
        console.warn(
    ...
    const initTilt = () => {
      ...
      import_vanilla_tilt.default.init(cardElement, {
        max: settings.max,
  ...
  // src/utils/profile-card-toggle.ts
  init_live_reload();
  var PROFILE_CARD_SELECTOR = ".profile-card_wrapper";
  var ACTIVE_CLASS = "active_fill";
  var ADDITIONAL_TOGGLE_ELEMENTS = [
    { selector: ".visual-block-card", activeClass: "active" },
    { selector: ".shadow-yellow", activeClass: "active" }
  ];
  var toggleAllCards = (isActive) => {
    const cards = document.querySelectorAll(PROFILE_CARD_SELECTOR);
    cards.forEach((card) => {
      if (isActive) {
  ...
  function createBezierPath(points, closed) {
  ...
  function generateRoundedStar(config) {
    const { width, height, color, opacity, random } = config;
    const points = random.nextInt(4, 12);
    const scale = random.nextFloat(0.6, 1);
    const rotation = random.nextFloat(0, 360);
    const innerRadiusRatio = random.nextFloat(0.3, 0.6);
    const pointDistortion = random.nextFloat(0, 0.3);
    const roundness = random.nextFloat(0.1, 0.4);
  ...
  function generateVariantStar(config) {
    const { width, height, color, opacity, random } = config;
    const points = random.nextInt(5, 10);
    const scale = random.nextFloat(0.65, 0.95);
    const rotation = random.nextFloat(0, 360);
    const innerRadiusRatio = random.nextFloat(0.35, 0.55);
    const pointDistortion = random.nextFloat(0.1, 0.4);
  ...
  function generateDistortedStar(config) {
    const { width, height, color, opacity, random } = config;
    const points = random.nextInt(4, 8);
    const scale = random.nextFloat(0.7, 1);
    const rotation = random.nextFloat(0, 360);
    const innerRadiusRatio = random.nextFloat(0.25, 0.5);
    const pointDistortion = random.nextFloat(0.2, 0.6);
  ...
  var SVGIllustrationGenerator = class {
    config;
    random;
    shapeGenerators;
    /**
      ...
      try {
        ...
        svgElements.forEach((svgElement, index) => {
          try {
            ...
            this.handleError(`Failed to regenerate element ${index + 1}`, error);
          }
        });
        const endTime = performance.now();
    ...
    replaceIllustrationElements() {
      const elements = document.querySelectorAll(this.config.illustrationSelector);
      ...
      elements.forEach((element, index) => {
        try {
          ...
          this.handleError(`Failed to replace element ${index + 1}`, error);
        }
      });
    }
    /**
     ...
     */
    createSVGElement(hasRotationClass) {
      const { viewBox, brandColor, opacityRange } = this.config;
    ...
    selectAndGenerateShape(config, shapeIndex) {
      try {
        ...
        this.handleError("Shape generation failed, falling back to circle", error);
  ...
  // node_modules/.pnpm/swiper@12.0.2/node_modules/swiper/swiper.mjs
  init_live_reload();
  ...
  var SwiperController = class {
     ...
     */
    isTypebotCompletion(data) {
      return typeof data === "object" && data !== null && "type" in data && data.type === "typebot-completion";
    }
    /**
     * Handle Typebot completion event
     */
    handleTypebotCompletion() {
  ...
  function initSwiperController(config = {}) {
  ...
  var TypebotEmailHandler = class {
    ...
    handleMessage(event) {
      try {
        ...
        if (this.isTypebotVariableUpdate(data)) {
          this.handleTypebotVariableUpdate(data);
        }
      } catch (error) {
     ...
     */
    isTypebotVariableUpdate(data) {
      return typeof data === "object" && data !== null && "type" in data && data.type === "typebot-variable-update" && "variable" in data;
    }
    /**
     * Handle Typebot variable update event
     */
    handleTypebotVariableUpdate(message) {
  ...
  function initTypebotEmailHandler(config = {}) {
  ...
  var TypebotNameReplacer = class {
    ...
    handleMessage(event) {
      try {
        const { data } = event;
        if (this.isTypebotCompletion(data)) {
          this.handleTypebotCompletion(data);
        }
        if (this.isTypebotVariableUpdate(data)) {
          this.handleTypebotVariableUpdate(data);
        }
     ...
     */
    isTypebotVariableUpdate(data) {
      return typeof data === "object" && data !== null && "type" in data && data.type === "typebot-variable-update" && "variable" in data && data.variable === "nome";
    }
    /**
     * Handle Typebot completion event
     */
    handleTypebotCompletion(message) {
  ...
  function initTypebotNameReplacer(config = {}) {
  ...
  window.Webflow.push(() => {
    ...
    initCardUpdates(cardMapper, 3e3);
    ...
    initProfileCardTilt({
      max: 15,
      // Subtle tilt rotation (15 degrees max)
      speed: 400,
